...
const generateToolObject = (
  ...
  const timeoutVal =
    typeof tool.timeout === 'number' && Number.isFinite(tool.timeout)
      ? tool.timeout
      : DEFAULT_TIMEOUT_MS;
  const permissionToolNamesJson = JSON.stringify(
    shellBasename === 'powershell' || shellBasename === 'pwsh'
      ? ['PowerShell']
      : shellBasename === 'cmd'
        ? ['Cmd', 'Command Prompt']
...
checkPermissions(input,context){
  ...
  let cmd=${cmdJson};
  ${inputSubst}
  const permissionToolNames=${permissionToolNamesJson};
  const permissionTool=context.options.tools.find(t=>permissionToolNames.includes(t.name));
  if(permissionTool)return permissionTool.checkPermissions({command:cmd,timeout:${timeoutVal}},context);
  return Promise.resolve({behavior:"passthrough",message:"Permission required to run "+${nameJson}+" with "+${shellJson}});
},
...
async call(args){
  ...
  let cmd=${cmdJson};
  ${argsSubst}
  const {spawn}=${requireFunc}("child_process");
  return await new Promise(resolve=>{
    const child=spawn(${shellJson},[${shellFlagJson},cmd],{
      encoding:"utf8",
      timeout:${timeoutVal},
      cwd:${workingDirExpr},
    ...
    child.on("close",code=>finish({stdout:stdout.trimEnd(),stderr:stderr.trimEnd(),exitCode:code??-1}));
  });
},
...
export const writeCustomTools = (
  ...
  for (const tool of customTools) {
    ...
    if (BUILTIN_TOOL_NAMES.has(normalizedName)) {
      console.error(
