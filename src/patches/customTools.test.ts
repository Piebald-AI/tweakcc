...
const buildGeneratedTool = (
  tool: CustomTool
): { tool: GeneratedTool; spawn: ReturnType<typeof vi.fn> } => {
...
describe('writeCustomTools', () => {
  ...
  describe('collision guard', () => {
    ...
    it('normalizes built-in tool names before collision checks', () => {
      const err = vi.spyOn(console, 'error').mockImplementation(() => {});
      try {
        const result = writeCustomTools(MOCK_STRATEGY_B, [
          { ...MINIMAL_TOOL, name: ' bash ' },
    ...
    it('normalizes duplicate custom tool names before collision checks', () => {
      const err = vi.spyOn(console, 'error').mockImplementation(() => {});
      try {
        const result = writeCustomTools(MOCK_STRATEGY_B, [
          MINIMAL_TOOL,
          { ...MINIMAL_TOOL, name: ' mytool ' },
        ]);
        expect(result).toBeNull();
        expect(err).toHaveBeenCalledWith(
          expect.stringContaining('duplicate custom tool name " mytool "')
  ...
  describe('Strategy B — original code injection', () => {
    ...
    it('delegates checkPermissions to the matching shell permission tool', () => {
      const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
      try {
        const shResult = writeCustomTools(MOCK_STRATEGY_B, [MINIMAL_TOOL])!;
        expect(shResult).toContain('const permissionToolNames=["Bash"]');
        expect(shResult).toContain('permissionTool.checkPermissions(');

        const pwshResult = writeCustomTools(MOCK_STRATEGY_B, [
          { ...MINIMAL_TOOL, shell: 'pwsh' },
        ])!;
        expect(pwshResult).toContain(
          'const permissionToolNames=["PowerShell"]'
        );
      } finally {
        warn.mockRestore();
      }
    ...
    it('emits input schema properties for __proto__ parameters', () => {
      const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
      try {
        const result = writeCustomTools(MOCK_STRATEGY_B, [
          {
            ...
            parameters: {
              ['__proto__']: {
                type: 'string',
                description: 'Prototype-like key',
                required: true,
              },
            },
            command: 'echo {{__proto__}}',
          },
        ])!;
        expect(result).toContain(
          '"__proto__":{"type":"string","description":"Prototype-like key"}'
        );
      } finally {
        warn.mockRestore();
      }
  ...
  describe('generated tool runtime', () => {
    ...
    it('treats $ replacement sequences literally in parameter values', async () => {
      const { tool, spawn } = buildGeneratedTool(MINIMAL_TOOL);
      const value = "$& $$ $` $'";
    ...
    it('falls back to the default timeout for non-finite timeout values', async () => {
      const { tool, spawn } = buildGeneratedTool({
        ...MINIMAL_TOOL,
        timeout: Number.NaN,
      });
    ...
    it('returns stderr and exitCode -1 when spawn emits an error', async () => {
      const { tool, spawn } = buildGeneratedTool(MINIMAL_TOOL);
      spawn.mockReturnValueOnce(
        createMockChild({ error: new Error('ETIMEDOUT') })
      );
    ...
    it('escapes regex-special parameter names in substitutions', async () => {
      const { tool, spawn } = buildGeneratedTool(SPECIAL_PARAM_TOOL);
